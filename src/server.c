...
void *pre_login_srv(void *param)
{
    ...
    while (1)
    {
        ...
        case QUIT_REQ:
            close(conn_socket);
            printf("You quited. Thank you for your using\n");
            pthread_exit(NULL);
        }
    }
}
...
void handle_signup(int conn_socket, Account *acc_list)
{
    ...
    if (target_acc)
    {
        printf("Account existed. Register fail!!!\n");
        result = ERR_ACC_EXISTED;
    }
    else
    {
...
void handle_activate(int conn_socket, Account *acc_list)
{
    ...
    if (result == ACC_IDLE)
    {
        ...
        if (!x)
        {
            ...
            pkg.ctrl_signal = ERR_ACTIVATE_FAIL;
        send(conn_socket, &pkg, sizeof(pkg), 0);
    }
    else
    {
...
void sv_search_user(int conn_socket)
{
    Package pkg;
    pkg.ctrl_signal = ERR_INCORRECT_ACC;
    recv(conn_socket, &pkg, sizeof(pkg), 0);
    Account *acc = find_account(acc_list, pkg.msg);
    if (acc != NULL)
    {
...
void sv_change_password(int conn_socket, Package *pkg)
{
    ...
    {
        pkg->ctrl_signal = ERR_WRONG_PASSWORD;
        send(conn_socket, pkg, sizeof(*pkg), 0);
        printf("0");
        return;
    }
    ...
    {
        pkg->ctrl_signal = ERR_CHANGE_PASSWORD_FAIL;
        return;
    }
